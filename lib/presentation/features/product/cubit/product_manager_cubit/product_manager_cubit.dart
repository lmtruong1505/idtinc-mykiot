...
import 'product_manager_state.dart';
...
class ProductManagerCubit extends Cubit<ProductManagerState> {
  ...
  ) : super(const ProductManagerState());
  ...
  final InfiniteListController<VariantEntity> productLibraryILC =
      InfiniteListController<VariantEntity>.init();
  final InfiniteListController<ProductEntity> productsILC =
      InfiniteListController<ProductEntity>.init();
  ...
  Future<List<ProductEntity>> getProducts(int page) async {
    ...
    final input = ProductListInput(
      ...
      page: page + 1,
      company: company,
      active: state.selectFilter == ProductStatus.all
          ? null
          : state.selectFilter == ProductStatus.active,
    );
    final res = await _productListUseCase.execute(input);
    emit(state.copyWith(productCount: res.response.extra));
  ...
  void selectFilterButton(ProductStatus value) {
