...
class ProdPictureView extends StatelessWidget {
  const ProdPictureView({
    super.key,
                  ...
                  children: [
                    ...
                    InkWell(
                      onTap: () async {
                        ...
                        final images = await picker.pickMultiImage(limit: 4 - state.imagesProduct.length);
                        myBloc.imageProductChange(images);
                      },
                      child: Row(
                        children: [
                          Expanded(
                            child: DottedBorder(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Text(
                                    'Tải ảnh lên ${state.imagesProduct.length}/4',
  ...
  Widget _buildListPic(ProductCreateState state){
    return Visibility(
      visible: state.imagesProduct.isNotEmpty,
      child: Container(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            final image = state.imagesProduct[index];
            return Stack(
              children: [
                Container(
                  width: sp56,
          ...
          itemCount: state.imagesProduct.length,
        ),
      ),
    );
  }
