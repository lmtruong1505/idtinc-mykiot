...
@RoutePage()
class PrepareListPage extends StatefulWidget {
  @override
  State<PrepareListPage> createState() => _PrepareListPageState();
}

class _PrepareListPageState extends State<PrepareListPage> {
  final myBloc = getIt.get<PrepareCubit>();
  @override
  Widget build(BuildContext context) => Scaffold(
      backgroundColor: bg_5,
      appBar: BaseAppBar(title: 'Danh sách dạng bào chế', actions: [
        InkWell(
            onTap: () async {},
            child: Container(
        ...
        InkWell(
            onTap: () async {
              await context.router.push(PrepareUpdateRoute());
  ...
  Widget _oneItem(PrepareEntity item) => GestureDetector(
        onTap: () async {
          await context.router.push(PrepareDetailRoute(id: item.id!));
          myBloc.entityILC.onRefresh();
        },
        child: Container(
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Expanded(
                  child: Column(
                      ...
                      children: [
                        Text('Dạng thuốc tiêm',
                            style: p5.copyWith(color: blackColor)),
                        Text(item.name,
                            overflow: TextOverflow.clip,
                            style: p6.copyWith(color: borderColor_3)),
                      ]),
                ),
                Container(
                    width: 32,
  ...
  MenuEntry _getMenus(int id) => MenuEntry(
          ...
          menuChildren: <MenuEntry>[
            MenuEntry(
                label: 'Sửa dạng bào chế',
                onPressed: () =>
                    context.router.push(PrepareUpdateRoute(id: id))),
            MenuEntry(
                label: 'Xoá dạng bào chế',
