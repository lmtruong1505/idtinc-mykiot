...
@LazySingleton(as: ProductStandardRepository)
class ProductStandardRepositoryImpl extends ProductStandardRepository {
  ProductStandardRepositoryImpl(this._dio);
  ...
  Future<dynamic> getList(int company, String search, int page) async {
    try {
      ...
      final res = await _dio.get(Api.productionStandardList, data: query);
  ...
  Future<dynamic> getDetail(int id) async {
    try {
      final res = await _dio.get("${Api.productionStandardDetail}$id");
      final dataRes = res.data['details'];
      if (dataRes != null) return dataRes;
    } catch (e) {
  ...
  Future<BaseResponseModel> create(Map<String, Object> data) async {
    try {
      final res = await _dio.post(Api.productionStandardCreate, data: data);
  ...
  Future<BaseResponseModel> update(int id, Map<String, Object> data) async {
    try {
      final res =
          await _dio.put('${Api.productionStandardDetail}$id', data: data);
      return BaseResponseModel(
          code: res.data['code'], message: res.data['message']);
    } catch (e) {
      if (kDebugMode) print(e);
  ...
  Future<BaseResponseModel> delete(int id) async {
    try {
      final res = await _dio.delete("${Api.productionStandardDetail}$id");
      return BaseResponseModel(
          code: res.data['code'], message: res.data['message']);
    } catch (e) {
      if (kDebugMode) print(e);
