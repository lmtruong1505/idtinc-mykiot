...
@RoutePage()
class RegisterCompanyDetailPage extends StatefulWidget {
  final int id;
  const RegisterCompanyDetailPage({super.key, required this.id});

  @override
  State<RegisterCompanyDetailPage> createState() =>
      _RegisterCompanyDetailPageState();
}

class _RegisterCompanyDetailPageState extends State<RegisterCompanyDetailPage> {
  final myBloc = getIt.get<RegisterCompanyCubit>();
  @override
  Widget build(BuildContext context) => Scaffold(
        appBar: BaseAppBar(title: 'Chi tiết công ty đăng ký', actions: [
          InkWell(
              onTap: () async {
                await context.router
                    .push(RegisterCompanyUpdateRoute(id: widget.id));
                ...
                child: BlocBuilder<RegisterCompanyCubit, RegisterCompanyState>(
                  ...
                  return SingleChildScrollView(
                    ...
                    child: Column(children: [
                      Container(
                          ...
                          child: Column(children: [
                            Container(
                                ...
                                child: Text('Thông tin công ty đăng ký',
                                    style: p3)),
                            Divider(height: 15, color: borderColor_2),
                            gapHeight(sp12),
                            Container(
                                alignment: Alignment.centerLeft,
                                child: Text('Mã công ty', style: p3)),
                            Container(
                                alignment: Alignment.centerLeft,
                            ...
                            gapHeight(sp12),
                            RowItem(
                                title: 'Địa chỉ đăng ký', content: "Việt Nam"),
                            gapHeight(sp12),
                            RowItem(
                                title: 'Người tạo',
                      ...
                      gapHeight(sp16),
                      BlocBuilder<RegisterCompanyCubit, RegisterCompanyState>(
                          builder: (context, state) {
                        return SingleChildScrollView(
                            controller: myBloc.scrollController,
                            child: Column(children: [
