...
class _SupplierGroupPageState extends State<SupplierGroupPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
      ...
      appBar: const BaseAppBar(title: 'Danh sách nhà cung cấp'),
      body: Container(
  ...
  Widget _oneItem(InventoryEntity item) => Container(
      ...
      child: Column(children: [
        ...
        gapHeight(sp16),
        RowItem(title: 'Số lượng tồn', content: '${item.amount}'),
        gapHeight(sp12),
        RowItem(
            title: 'ĐVT cơ bản', content: '${item.variant?.units?[0].name}'),
        gapHeight(sp12),
        RowItem(title: 'Giá trị tồn', content: '${item.price} VNĐ'),
      ]));
}
