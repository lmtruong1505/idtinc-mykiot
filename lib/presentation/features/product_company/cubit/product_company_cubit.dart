...
import 'package:pharmago/presentation/features/product_company/domain/usecase/product_company_use_case.dart';
...
@injectable
class ProductCompanyCubit extends Cubit<ProductCompanyState> {
  ProductCompanyCubit(this._useCase, this._productListUseCase)
      : super(const ProductCompanyState());

  final ProductCompanyUseCase _useCase;
  final ProductListUseCase _productListUseCase;
  final entityILC = InfiniteListController<ProductCompanyEntity>.init();
  final productsILC = InfiniteListController<ProductEntity>.init();
  final ScrollController scrollController = ScrollController();

  Future<List<ProductCompanyEntity>> getList(int page) async {
    final company =
  ...
  Future<void> getDetail(BuildContext context, int? id) async {
    final item =
        id == null ? ProductCompanyEntity() : await _useCase.getDetail(id);
    final company =
        AppSharedPreference.instance.getValue(PrefKeys.company) as int?;
  ...
  void create(BuildContext context) {
    DialogUtils.showDialogWithTitleAndOptionButton(
        context, "Xác nhận tạo công ty sản xuất??", () async {
  ...
  void update(BuildContext context) {
    DialogUtils.showDialogWithTitleAndOptionButton(
        context, "Xác nhận update công ty sản xuất??", () async {
      ...
      if (res.code == 200) {
        ...
        await DialogUtils.showErrorDialog(context,
            content: 'Cập nhật công ty sản xuất thất bại');
      }
    });
  }
  ...
  void delete(BuildContext context, int id, bool back) {
    DialogUtils.showDialogWithTitleAndOptionButton(context,
        "Bạn có chắc muốn xóa công ty sản xuất?\nThao tác này sẽ không thể hoàn tác",
        () async {
      DialogUtils.showLoadingDialog(
          context, 'Đang xoá công ty sản xuất vui lòng đợi!');
      final res = await _useCase.delete(id);
      Navigator.of(context).pop();
      if (res.code == 200) {
        await DialogUtils.showSuccessDialog(context,
            content: 'Xoá công ty sản xuất thành công', barrierDismissible: true,);
        if (back) {
          Navigator.of(context).pop();
        } else {
          entityILC.onRefresh();
        ...
        await DialogUtils.showErrorDialog(context,
            content: 'Xoá công ty sản xuất thất bại');
      }
    });
  }
}
