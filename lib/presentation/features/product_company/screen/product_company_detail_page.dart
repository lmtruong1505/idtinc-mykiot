...
@RoutePage()
class ProductCompanyDetailPage extends StatefulWidget {
  final int id;
  const ProductCompanyDetailPage({super.key, required this.id});

  @override
  State<ProductCompanyDetailPage> createState() =>
      _ProductCompanyDetailPageState();
}

class _ProductCompanyDetailPageState extends State<ProductCompanyDetailPage> {
  final myBloc = getIt.get<ProductCompanyCubit>();
  @override
  Widget build(BuildContext context) => Scaffold(
        appBar: BaseAppBar(title: 'Chi tiết công ty sản xuất', actions: [
          InkWell(
              onTap: () async {
                await context.router
                    .push(ProductCompanyUpdateRoute(id: widget.id));
                ...
                child: BlocBuilder<ProductCompanyCubit, ProductCompanyState>(
                  ...
                  return SingleChildScrollView(
                    ...
                    child: Column(children: [
                      Container(
                          ...
                          child: Column(children: [
                            Container(
                                ...
                                child: Text('Thông tin công ty sản xuất',
                                    style: p3)),
                            Divider(height: 15, color: borderColor_2),
                            gapHeight(sp12),
                            Container(
                                alignment: Alignment.centerLeft,
                                child: Text('Mã công ty: ${state.item.code}',
                                    style: p3)),
                            Container(
                                alignment: Alignment.centerLeft,
                            ...
                            gapHeight(sp12),
                            RowItem(
                                title: 'Nước sản xuất', content: "Việt Nam"),
                            gapHeight(sp12),
                            RowItem(
                                title: 'Người tạo',
                            ...
                            RowItem(
                                ...
                                content: state.item.userUpdatedName),
                            gapHeight(sp12),
                            RowItem(
                                title: 'Thời gian cập nhật',
                                content: state.item.updatedAt),
                            gapHeight(sp12),
                          ])),
                      gapHeight(sp28),
                      Container(
                      ...
                      gapHeight(sp16),
                      BlocBuilder<ProductCompanyCubit, ProductCompanyState>(
                          builder: (context, state) {
                        return SingleChildScrollView(
                            controller: myBloc.scrollController,
                            child: Column(children: [
