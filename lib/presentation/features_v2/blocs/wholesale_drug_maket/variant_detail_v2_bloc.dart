...
@injectable
class VariantDetailV2Bloc extends Cubit<CubitState<VariantKafaModel>> {
  VariantDetailV2Bloc(this._repo, this.vouchersRepo) : super(CubitState());

  final VariantKafaRepository _repo;
  final CartVouchersRepo vouchersRepo;

  int _countCTKM = 0;
  int get countCTKM => _countCTKM;

  int _countQuantity = 0;
  int get countQuantity => _countQuantity;
  ...
  num get totalDiscount {
    ...
    return (typeDiscount == 1) //==1 tiền mặt ,==2 %
        ...
        : totalPrice * (promotionActive?.discount ?? 0) / 100;
  }
  ...
  PromotionVoucherItem? get promotionActive {
    final items = voucherSelected?.promotionItems;
  ...
  void getDetail(int variant) async {
    emit(state.copyWith(status: BlocStatus.loading));
    final res = await _repo.getDetail(id: variant);
    final listFromShoppingCart = getIt.get<ShoppingCartBloc>().list;
    final idx =
        listFromShoppingCart.indexWhere((element) => element.id == variant);
    if (idx == -1) {
      emit(state.copyWith(status: BlocStatus.success, data: res.data));
      // getPromotion(variant);
      return;
    }
    emit(state.copyWith(
        status: BlocStatus.success, data: listFromShoppingCart[idx]));
    getCountCTKM();
    getCountQuantity();
  }
